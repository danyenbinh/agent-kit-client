...
export async function applyPacksToProject(projectRoot, opts = {}) {
  ...
  for (const packId of packs) {
    try {
      ...
      // Per-pack Unity MCP modules — merge overlay (host + addons). Never wipe on addon-only packs.
      ...
      if (fs.existsSync(mcpSrc)) {
        ...
        ensureDir(mcpDst);
        if (packId === "unity-runtime") {
          // Host replace keeps node_modules if present
          const nm = path.join(mcpDst, "node_modules");
          const keepNm = fs.existsSync(nm) ? nm + ".__keep" : null;
          if (keepNm) fs.renameSync(nm, keepNm);
          for (const name of fs.readdirSync(mcpDst)) {
            if (name === "node_modules" || name.endsWith(".__keep")) continue;
            rmrf(path.join(mcpDst, name));
          }
          if (keepNm && fs.existsSync(keepNm)) fs.renameSync(keepNm, nm);
        }
        for (const name of fs.readdirSync(mcpSrc)) {
          if (name === "node_modules") continue;
          const s = path.join(mcpSrc, name);
          const d = path.join(mcpDst, name);
          if (fs.statSync(s).isDirectory()) copyDir(s, d);
          else {
            ensureDir(path.dirname(d));
            fs.copyFileSync(s, d);
